...
test: .FORCE
	@if test -n '$(PHPUNIT)'; then set -- $(PHPUNIT); elif test -x vendor/bin/phpunit; then set -- vendor/bin/phpunit; else set -- phpunit; fi; set -- "$$@" --test-suffix .php --testdox -vv $(TESTS_DIR); echo "$$@"; "$$@"
